...
const pagePath = new URL("../app/LegacyNotebook.tsx", import.meta.url);
...
test("session cards can edit questions before reveal and explanations after reveal", async () => {
  ...
  assert.match(source,/問題文\s*<textarea\s+autoFocus/);
  assert.match(source,/解説文\s*<textarea\s+autoFocus/);
});
...
test("inline saves reuse the shared card API and keep the session card in place", async () => {
  ...
  assert.match(source,/position === index \? \{\s*\.\.\.card,\s*\.\.\.draft\s*\} : card/);
