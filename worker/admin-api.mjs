...
function validLessonId(value) {
  return (
    value === DEFAULT_LESSON_ID || /^lesson-[a-z0-9-]{10,80}$/i.test(value)
  );
}
function validCardId(value) {
  return /^card-[a-z0-9-]{10,80}$/i.test(value);
}
function validResourceId(value) {
  return /^resource-[a-z0-9-]{10,80}$/i.test(value);
}
...
function dateText(value) {
  return typeof value === "string" && /^\d{1,2}\/\d{1,2}$/.test(value.trim())
    ? value.trim()
    : "";
}
...
async function nextSortOrder(db, lessonId) {
  const result = await db
    .prepare(
      "SELECT COALESCE(MAX(sort_order), 0) + 1 AS next_order FROM notebook_cards WHERE lesson_id = ?",
    )
    .bind(lessonId)
    .all();
...
async function nextResourceSortOrder(db, lessonId) {
  const result = await db
    .prepare(
      "SELECT COALESCE(MAX(sort_order), 0) + 1 AS next_order FROM lesson_resources WHERE lesson_id = ?",
    )
    .bind(lessonId)
    .all();
  return Number(result.results?.[0]?.next_order ?? 1);
...
export async function handleAdminApi(request, env) {
  ...
  const imageMatch = url.pathname.match(
    /^\/api\/images\/(note-images\/[a-f0-9-]+\.(?:jpg|png|webp|gif))$/i,
  );
  if (imageMatch && request.method === "GET") {
    if (!env.MEDIA)
      return json(request, { error: "画像保存を利用できません。" }, 503);
    const object = await env.MEDIA.get(imageMatch[1]);
    if (!object) return json(request, { error: "画像が見つかりません。" }, 404);
    const imageHeaders = headers(request);
    imageHeaders.set(
      "Content-Type",
  ...
  if (url.pathname === "/api/images" && request.method === "POST") {
    ...
    if (!extension || (declaredSize && declaredSize > MAX_IMAGE_BYTES))
      return json(
        request,
        { error: "PNG・JPEG・WebP・GIFの8MB以下の画像を選んでください。" },
        400,
      );
    ...
    return json(request, {
      ok: true,
      url: `${url.origin}/api/images/${key}`,
      markdown: `![画像](${url.origin}/api/images/${key})`,
    });
  }
  if (!env.DB)
  ...
  if (url.pathname === "/api/notebook" && request.method === "GET") {
    const [legacy, metadata, lessons, cards, resources] = await Promise.all([
      env.DB.prepare(
        "SELECT lesson_id, card_id, question, answer, deleted, updated_at FROM lesson_card_overrides ORDER BY card_id",
      ).all(),
      env.DB.prepare(
        "SELECT lesson_id, lesson_date, teacher, title, video_url, updated_at FROM lesson_metadata_overrides",
      ).all(),
      env.DB.prepare(
    ...
    return json(request, {
      overrides: (legacy.results ?? []).map((row) => ({
      ...
      metadata: (metadata.results ?? []).map((row) => ({
        lessonId: row.lesson_id,
      ...
      lessons: (lessons.results ?? []).map((row) => ({
        id: row.lesson_id,
        date: row.lesson_date,
        teacher: row.teacher ?? "",
        title: row.title,
  ...
  if (url.pathname === "/api/cards" && request.method === "GET") {
    ...
    return json(request, {
      overrides: (result.results ?? []).map((row) => ({
        lessonId: row.lesson_id,
        id: row.card_id,
        question: row.question,
        answer: row.answer,
  ...
  const baseMatch = url.pathname.match(
    /^\/api\/admin\/cards\/([^/]+)\/(\d+)(\/delete)?$/,
  );
  if (baseMatch && baseMatch[1] === DEFAULT_LESSON_ID) {
    const id = Number(baseMatch[2]);
    if (!Number.isInteger(id) || id < 1 || id > MAX_BASE_CARD_ID)
      return json(request, { error: "対象のカードが見つかりません。" }, 404);
    if (baseMatch[3]) {
      if (request.method !== "DELETE")
        return json(request, { error: "この操作は削除専用です。" }, 405);
      await env.DB.prepare(
        "INSERT INTO lesson_card_overrides (lesson_id,card_id,question,answer,deleted,updated_at) VALUES (?,?,'','',1,CURRENT_TIMESTAMP) ON CONFLICT(lesson_id,card_id) DO UPDATE SET deleted=1,updated_at=CURRENT_TIMESTAMP",
      )
        .bind(DEFAULT_LESSON_ID, id)
        .run();
      return json(request, { ok: true, deleted: true });
    }
    if (request.method === "PUT") {
      const body = await parseBody(request);
      const question = text(body?.question, MAX_QUESTION_LENGTH);
      ...
      await env.DB.prepare(
        "INSERT INTO lesson_card_overrides (lesson_id,card_id,question,answer,deleted,updated_at) VALUES (?,?,?,?,0,CURRENT_TIMESTAMP) ON CONFLICT(lesson_id,card_id) DO UPDATE SET question=excluded.question,answer=excluded.answer,deleted=0,updated_at=CURRENT_TIMESTAMP",
      )
        .bind(DEFAULT_LESSON_ID, id, question, answer)
        .run();
      return json(request, { ok: true });
    }
    if (request.method === "DELETE") {
      await env.DB.prepare(
        "DELETE FROM lesson_card_overrides WHERE lesson_id = ? AND card_id = ? AND question='' AND answer=''",
      )
        .bind(DEFAULT_LESSON_ID, id)
        .run();
      await env.DB.prepare(
        "UPDATE lesson_card_overrides SET deleted=0,updated_at=CURRENT_TIMESTAMP WHERE lesson_id=? AND card_id=?",
      )
        .bind(DEFAULT_LESSON_ID, id)
        .run();
  ...
  if (url.pathname === "/api/lessons" && request.method === "POST") {
    ...
    const teacher = text(body?.teacher, MAX_TEACHER_LENGTH);
    const title = text(body?.title, MAX_LESSON_LENGTH);
    const video = videoUrl(body?.videoUrl);
    if (!date || !teacher || !title || (body?.videoUrl && !video))
      return json(
        request,
        { error: "日付、講師、タイトル、YouTubeリンクを確認してください。" },
        400,
      );
    const id = `lesson-${crypto.randomUUID()}`;
    await env.DB.prepare(
      "INSERT INTO notebook_lessons (lesson_id,lesson_date,teacher,title,video_url) VALUES (?,?,?,?,?)",
    )
      .bind(id, date, teacher, title, video)
      .run();
    return json(request, {
      ok: true,
      lesson: { id, date, teacher, title, videoUrl: video },
    });
  }

  const restoreLesson = url.pathname.match(
    /^\/api\/lessons\/([^/]+)\/restore$/,
  );
  if (restoreLesson && request.method === "POST") {
    if (
      !validLessonId(restoreLesson[1]) ||
      restoreLesson[1] === DEFAULT_LESSON_ID
    )
      return json(request, { error: "対象の授業が見つかりません。" }, 404);
    await env.DB.prepare(
      "UPDATE notebook_lessons SET deleted=0,updated_at=CURRENT_TIMESTAMP WHERE lesson_id=?",
    )
      .bind(restoreLesson[1])
      .run();
    return json(request, { ok: true, restored: true });
  }
  ...
  if (lessonMatch) {
    ...
    if (request.method === "PUT") {
      ...
      if (lessonId === DEFAULT_LESSON_ID)
        await env.DB.prepare(
          "INSERT INTO lesson_metadata_overrides (lesson_id,lesson_date,teacher,title,video_url,updated_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(lesson_id) DO UPDATE SET lesson_date=excluded.lesson_date,teacher=excluded.teacher,title=excluded.title,video_url=excluded.video_url,updated_at=CURRENT_TIMESTAMP",
        )
          .bind(lessonId, date, teacher, title, video)
          .run();
      else
        await env.DB.prepare(
          "UPDATE notebook_lessons SET lesson_date=?,teacher=?,title=?,video_url=?,updated_at=CURRENT_TIMESTAMP WHERE lesson_id=?",
        )
          .bind(date, teacher, title, video, lessonId)
          .run();
      return json(request, { ok: true });
    }
    if (request.method === "DELETE" && lessonId !== DEFAULT_LESSON_ID) {
      await env.DB.prepare(
        "UPDATE notebook_lessons SET deleted=1,updated_at=CURRENT_TIMESTAMP WHERE lesson_id=?",
      )
        .bind(lessonId)
        .run();
      return json(request, { ok: true, deleted: true });
  ...
  const resourceCollection = url.pathname.match(
  ...
  if (resourceCollection && request.method === "POST") {
    ...
    if (!validLessonId(lessonId))
      return json(request, { error: "対象の授業が見つかりません。" }, 404);
    const body = await parseBody(request);
    const kind = ["link", "image"].includes(body?.kind) ? body.kind : "link";
    const urlValue = videoUrl(body?.url);
    const label =
      text(body?.label, MAX_RESOURCE_LABEL_LENGTH) ||
      (kind === "image" ? "画像資料" : "参考資料");
    if (!urlValue)
      return json(
        request,
    ...
    const sortOrder = await nextResourceSortOrder(env.DB, lessonId);
    await env.DB.prepare(
      "INSERT INTO lesson_resources (resource_id,lesson_id,sort_order,kind,label,url) VALUES (?,?,?,?,?,?)",
    )
      .bind(id, lessonId, sortOrder, kind, label, urlValue)
      .run();
    return json(request, {
      ok: true,
      resource: { id, lessonId, sortOrder, kind, label, url: urlValue },
    });
  }
  ...
  if (
    resourceMatch &&
    validLessonId(resourceMatch[1]) &&
    validResourceId(resourceMatch[2])
  ) {
    ...
    if (request.method === "DELETE") {
      await env.DB.prepare(
        "DELETE FROM lesson_resources WHERE lesson_id=? AND resource_id=?",
      )
        .bind(lessonId, resourceId)
        .run();
      return json(request, { ok: true, deleted: true });
    }
  }
  ...
  if (cardCollection && request.method === "POST") {
    ...
    if (!validLessonId(lessonId))
      return json(request, { error: "対象の授業が見つかりません。" }, 404);
    const body = await parseBody(request);
    const kind = ["question", "section", "note"].includes(body?.kind)
      ? body.kind
      : "question";
    const question = text(body?.question, MAX_QUESTION_LENGTH);
    const answer = text(body?.answer, MAX_ANSWER_LENGTH);
  ...
  if (cardMatch && validLessonId(cardMatch[1]) && validCardId(cardMatch[2])) {
    ...
    if (restore) {
      if (request.method !== "POST")
        return json(request, { error: "この操作には対応していません。" }, 405);
      await env.DB.prepare(
        "UPDATE notebook_cards SET deleted=0,updated_at=CURRENT_TIMESTAMP WHERE lesson_id=? AND card_id=?",
      )
        .bind(lessonId, cardId)
        .run();
      return json(request, { ok: true, restored: true });
    }
    if (request.method === "PUT") {
    ...
    if (request.method === "DELETE") {
      await env.DB.prepare(
        "UPDATE notebook_cards SET deleted=1,updated_at=CURRENT_TIMESTAMP WHERE lesson_id=? AND card_id=?",
      )
        .bind(lessonId, cardId)
        .run();
      return json(request, { ok: true, deleted: true });
