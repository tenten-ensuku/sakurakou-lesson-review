...
type Lesson = { id:string; date:string; title:string; videoUrl:string; deleted?:boolean };
...
export default function Home() {
  ...
  const [activeLessonId,setActiveLessonId] = useState(DEFAULT_LESSON.id);
  const [adminLessonId,setAdminLessonId] = useState(DEFAULT_LESSON.id);
  ...
  const [lessonDraft,setLessonDraft] = useState({date:"",title:"",videoUrl:""});
  const [newLesson,setNewLesson] = useState({date:todayShort(),title:"",videoUrl:""});
  const [cardDrafts,setCardDrafts] = useState<Record<string,{kind:Kind;question:string;answer:string}>>({});

  const defaultLesson = useMemo(() => ({ ...DEFAULT_LESSON, ...(notebook.metadata.find((item) => item.id === DEFAULT_LESSON.id) ?? {}) }),[notebook.metadata]);
  const lessons = useMemo(() => sortLessons([defaultLesson,...notebook.lessons.filter((item) => !item.deleted)]),[defaultLesson,notebook.lessons]);
  ...
  const activeReview = reviewIds.filter((key) => activeQuestions.some((card) => cardKey(activeLesson.id,card) === key));
  ...
  useEffect(() => {
    ...
    setLessonDraft({date:lesson.date,title:lesson.title,videoUrl:lesson.videoUrl});
  ...
  const start = (review = false) => {
    const chosen = review ? activeQuestions.filter((card) => activeReview.includes(cardKey(activeLesson.id,card))) : activeCards;
    if (!chosen.length) return; setSessionCards(chosen); setSessionLesson(activeLesson); setIndex(0); setRevealed(false); setRatings({}); setElapsed(0); setResult(null); setScreen("session");
  };
  ...
  return <main className="app-shell"><div className="felt-grain" aria-hidden="true" />
    {screen === "home" && <section className="screen screen--home" aria-labelledby="app-title"><Header />
      ...
      <div className="lesson-grid">{lessons.map((lesson) => { const cards=cardsFor(lesson.id); const questions=cards.filter((card)=>card.kind === "question"); const reviews=reviewIds.filter((key)=>questions.some((card)=>cardKey(lesson.id,card)===key)).length; return <section className="mode-panel lesson-panel" key={lesson.id}><div className="section-heading"><div className="lesson-title-row"><h2><span className="lesson-date">{lesson.date}</span>{lesson.title}</h2>{lesson.videoUrl && <a className="youtube-icon-button" href={lesson.videoUrl} target="_blank" rel="noreferrer" aria-label="授業動画をYouTubeで見る"><span className="youtube-play-mark" /></a>}</div><span className="review-count">解き直し <strong>{reviews}</strong> 枚</span></div><p className="lesson-card-summary">全{questions.length}問＋学習カード{cards.length-questions.length}枚</p><div className="mode-grid"><button className="mode-card mode-card--primary" disabled={!cards.length} onClick={()=>{setActiveLessonId(lesson.id); setTimeout(()=>start(false),0);}}><span className="mode-card__number">{questions.length}</span><span><strong>すべて学習する</strong><small>問題と講義メモを順番に確認</small></span><span className="mode-card__arrow">→</span></button><button className="mode-card mode-card--review" disabled={!reviews} onClick={()=>{setActiveLessonId(lesson.id); setTimeout(()=>start(true),0);}}><span className="mode-card__number">↺</span><span><strong>解き直しカード</strong><small>{reviews ? `${reviews}枚を解き直す` : "回答後に追加できます"}</small></span><span className="mode-card__arrow">→</span></button></div><div className="lesson-panel-actions"><button className="text-button" onClick={()=>{setActiveLessonId(lesson.id);setScreen("list");}}>☰ カード一覧</button><button className="text-button" onClick={()=>{setAdminLessonId(lesson.id);setScreen("admin");}}>編集</button></div></section>; })}</div>
    </section>}
